...
import { LayoutProps } from "src/configs/g_types/types";
import { hexToRGBA } from "src/configs/theme/hex-to-rgba";
...
const LayoutAppBar = (props: Props) => {
  // ** Props
  const { settings, appBarProps } = props;
  ...
  const appBarBlurEffect = appBarBlur && {
    "&:after": {
      ...
      height: (theme: Theme) =>
        `calc(${theme.mixins.toolbar.minHeight as number}px + ${theme.spacing(
          4
        )})`,
      mask: (theme: Theme) =>
      ...
      <Toolbar
        ...
        sx={{
          ...
          ...(skin === "bordered"
            ...
            : { boxShadow: 4 }),
          ...(contentWidth === "boxed" && {
            "@media (min-width:1440px)": { maxWidth: (theme) => `calc(100%)` },
            // '@media (min-width:1440px)': { maxWidth: theme => `calc(100% - ${theme.spacing(6 * 2)})` }
          }),
        }}
      >
        <AppBarContent {...props} />
